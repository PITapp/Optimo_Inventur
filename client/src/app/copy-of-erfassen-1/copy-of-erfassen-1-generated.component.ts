...
this.dsoArtikelLaden.Artikelnummer = null;
...
this.dsoUpdateErfassen.GeloeschtAm = new Date(Date.UTC(date.getFullYear(),
  ...
  buttonArtikelLadenClick(event: any) {
    if (event.ArtikelID == 0 && event.Artikelnummer == null) {
      this.datalistErfassung.load();
    }
    ...
    if (event.ArtikelID != 0) {
      ...
      this.buttonBerechneArtikelSummeErfasstClick(result.ArtikelID);
          ...
          this.notificationService.notify({ severity: "error", summary: ``, detail: `Artikel für diesen Lagerort nicht gefunden!` });
      });
    }

    if (event.Artikelnummer != null) {
      ...
      .subscribe((result: any) => {
        ...
        this.notificationService.notify({ severity: "success", summary: ``, detail: `--- Artikel gefunden ---` });
      }
      ...
      if (result.value.length == 0) {
        this.notificationService.notify({ severity: "error", summary: ``, detail: `Artikel für diesen Lagerort nicht gefunden!` });
          ...
          this.notificationService.notify({ severity: "error", summary: ``, detail: `Fehler beim Laden des Artikels!` });
      });
    }
  }
